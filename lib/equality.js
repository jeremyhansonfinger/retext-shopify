 ...
 * @fileoverview Warn about possible insensitive, incorrect language
 *   with Retext.
 */
...
function simple(matches, pattern, file) {
    ...
    while (++index < length) {
        ...
        warn(file, id, toString(nodes), pattern.correct, nodes[0], note);
    }
}
...
function and(matches, pattern, file) {
    ...
    while (++index < length) {
        ...
        if (position !== -1) {
            ...
            suggestions.push(byValue(pattern.correct, match.type));
...
function or(matches, pattern, file) {
    ...
    while (++index < length) {
        ...
        warn(file, id, value, pattern.correct, nodes[0], note);
    }
}
